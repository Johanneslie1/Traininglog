...
jest.mock('@/services/firebase/sessionTrackingService', () => ({
  ...
  ensureDefaultSessionForDate: jest.fn(),
...
const sessionTrackingServiceMock = jest.requireMock('@/services/firebase/sessionTrackingService') as {
  ...
  ensureDefaultSessionForDate: jest.Mock;
};
...
describe('ExerciseLog session type filtering', () => {
  ...
  it('ensures baseline Session 1 and Warm-up 1 are created for selected date', async () => {
    render(<ExerciseLog />);

    await waitFor(() => {
      expect(sessionTrackingServiceMock.ensureDefaultSessionForDate).toHaveBeenCalledWith(
        'user-1',
        expect.any(Date),
        'main'
      );
