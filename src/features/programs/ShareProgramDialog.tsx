...
const ShareProgramDialog: React.FC<ShareProgramDialogProps> = ({
  programId,
  programName,
  onClose,
  onSuccess
  ...
  const handleShare = async () => {
    ...
    try {
      await shareProgram(programId, Array.from(selectedAthleteIds), coachMessage.trim() || undefined);
      toast.success(`Program shared with ${selectedAthleteIds.size} athlete${selectedAthleteIds.size !== 1 ? 's' : ''}`);
      onSuccess?.();
      onClose();
    } catch (err) {
      console.error('Error sharing program:', err);
      toast.error(err instanceof Error ? err.message : 'Failed to share program');
    } finally {
      setIsSharing(false);
    }
  };
      ...
      <div className="bg-bg-secondary rounded-lg max-w-2xl w-full shadow-2xl border border-border max-h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-text-primary">Share Program</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6">
          ...
          <div className="bg-bg-tertiary rounded-lg p-4">
            <p className="text-text-secondary mb-1">
              Sharing: <span className="font-semibold text-text-primary">{programName}</span>
            </p>
...
export default ShareProgramDialog;
