
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { usePrograms } from '@/context/ProgramsContext';
import ProgramModal from './ProgramModal';

const levelColors: Record<string, string> = {
  Any: 'bg-gray-500',
  Beginner: 'bg-green-600',
  Intermediate: 'bg-yellow-600',
  Advanced: 'bg-red-600',
};

const ProgramList: React.FC<{ onSelect?: (id: string) => void }> = ({ onSelect }) => {
  console.log('[ProgramList] MOUNTED');
  const { programs, create } = usePrograms();
  const [showModal, setShowModal] = useState(false);
  const navigate = useNavigate();

  const handleAdd = async (data: { name: string; level: string; description: string }) => {
    const newId = crypto.randomUUID();
    const newProgram = {
      id: newId,
      name: data.name,
      description: data.description,
      level: data.level,
      createdBy: 'user', // TODO: Replace with actual user id from auth
      createdAt: new Date().toISOString(),
      routines: [],
    };
    console.log('[ProgramList] handleAdd called with:', newProgram);
    if (!create) {
      console.error('[ProgramList] create function is undefined!');
      return;
    }
    if (!showModal) {
      console.warn('[ProgramList] showModal is false when trying to add!');
    }
    try {
      await create(newProgram);
      console.log('[ProgramList] Program created, navigating to:', `/programs/${newId}`);
    } catch (err) {
      console.error('[ProgramList] Error in handleAdd:', err);
    }
    setShowModal(false);
    navigate(`/programs/${newId}`);
  };

  return (
    <div className="relative min-h-screen pb-20">
      <h2 className="text-xl font-bold mb-4">Programs</h2>
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
        {programs.map(program => (
          <div
            key={program.id}
            className="relative bg-[#23272F] rounded-xl p-4 flex flex-col items-start justify-between min-h-[120px] shadow-md cursor-pointer overflow-hidden"
            onClick={() => (onSelect ? onSelect(program.id) : navigate(`/programs/${program.id}`))}
          >
            <div className="absolute right-3 top-3 opacity-20 text-5xl pointer-events-none select-none">
              <span role="img" aria-label="kettlebell">🏋️‍♂️</span>
            </div>
            <div className="font-semibold text-lg text-white mb-1 z-10">{program.name}</div>
            <div className="flex items-center gap-2 z-10">
              <span className={`px-2 py-0.5 rounded text-xs text-white ${levelColors[program.level] || 'bg-gray-700'}`}>{program.level || 'Any'}</span>
            </div>
            {program.description && <div className="text-gray-400 text-xs mt-2 z-10 line-clamp-2">{program.description}</div>}
          </div>
        ))}
      </div>
      <button
        className="fixed bottom-6 right-6 bg-blue-600 hover:bg-blue-500 text-white rounded-full w-14 h-14 flex items-center justify-center text-3xl shadow-lg z-50"
        onClick={() => setShowModal(true)}
        aria-label="Add Program"
      >
        +
      </button>
      <ProgramModal isOpen={showModal} onClose={() => setShowModal(false)} onSave={handleAdd} />
    </div>
  );
};

export default ProgramList;
