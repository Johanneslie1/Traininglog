...
type Counters = {
  usersScanned: number;
  logsScanned: number;
  logsAlreadyBacktracked: number;
  logsChanged: number;
  fieldsReduced: number;
  fieldsClampedAtOne: number;
  unexpectedValues: number;
  writesApplied: number;
};

const DATABASE_ID = '(default)';
const BACKTRACK_MARKER = 'wellnessLoadMetricsReducedByOne';
const LOAD_METRIC_FIELDS = ['fatigue', 'stress', 'muscleSoreness'] as const;
...
async function run() {
  ...
  if (!token) {
    throw new Error('Could not resolve Firebase access token');
  }
  const counters: Counters = {
    usersScanned: 0,
    logsScanned: 0,
    logsAlreadyBacktracked: 0,
    logsChanged: 0,
    fieldsReduced: 0,
    fieldsClampedAtOne: 0,
    unexpectedValues: 0,
    writesApplied: 0,
  };
  ...
  for (const userDoc of users) {
    ...
    for (const logDoc of wellnessLogs) {
      ...
      if (readBoolean(logDoc.fields, BACKTRACK_MARKER)) {
        counters.logsAlreadyBacktracked += 1;
        continue;
      }

      const updateFields: Record<string, FirestoreValue> = {
        [BACKTRACK_MARKER]: { booleanValue: true },
        wellnessLoadMetricsReducedAt: { timestampValue: new Date().toISOString() },
      };
      let changed = false;

      LOAD_METRIC_FIELDS.forEach((field) => {
        ...
        const reduced = Math.max(1, rounded - 1);
        updateFields[field] = { integerValue: String(reduced) };

        if (reduced !== currentValue) {
          counters.fieldsReduced += 1;
          changed = true;
          if (reduced === 1 && currentValue <= 1) {
            counters.fieldsClampedAtOne += 1;
          }
        }
      });
  ...
  console.log(JSON.stringify({
    ...
    projectId,
    reducedFields: LOAD_METRIC_FIELDS,
    reduction: '-1, clamped at 1',
    idempotencyMarker: BACKTRACK_MARKER,
    counters,
  }, null, 2));
...
run().catch((error) => {
  console.error('[backtrackWellnessLoadMetrics] Failed:', error);
  process.exitCode = 1;
});
