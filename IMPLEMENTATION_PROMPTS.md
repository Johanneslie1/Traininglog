...
### Prompt 4.4: Fix Settings Modal on Program Routes

**Problem:** Settings button in SideMenu doesn't work on `/programs` and `/programs/:id` routes.

**Root Cause Analysis:**
1. `Layout.tsx` manages the Settings modal state (`showSettings`) and correctly opens it via SideMenu
2. However, on program routes, `isProgramRoute` check prevents Layout from rendering its SideMenu
3. `ProgramDetail.tsx` and `ProgramList.tsx` render their own local SideMenu instances
4. These local SideMenus don't have access to Layout's Settings state
5. Clicking Settings just navigates to `/` instead of opening the modal

**Current Behavior:**
```typescript
// In ProgramDetail.tsx and ProgramList.tsx
onOpenSettings={() => {
  setShowSideMenu(false);
  navigate('/');  // ❌ Wrong - just navigates away
}}
```
...
```
Fix the Settings modal so it opens correctly from all SideMenu instances throughout the app.

Current Issue:
- Settings button works in Layout.tsx's SideMenu (on home route)
- Settings button DOESN'T work in ProgramDetail.tsx and ProgramList.tsx SideMenus
- Clicking Settings in programs just navigates to '/' without opening the modal

Files Affected:
1. `src/features/programs/ProgramDetail.tsx`
2. `src/features/programs/ProgramList.tsx`
3. `src/features/exercises/ExerciseLog.tsx` (same issue)

Solution:
1. Add local Settings modal state to each component that renders SideMenu
2. Import Settings component in each file
3. Update onOpenSettings handler to set local state instead of navigating
4. Render Settings modal with isOpen={showSettings} in each component

Implementation Steps:

**Step 1: ProgramDetail.tsx**
```typescript
// Add import
...
// Update SideMenu onOpenSettings (around line 531)
onOpenSettings={() => {
  setShowSideMenu(false);
  setShowSettings(true);  // ✅ Open local Settings modal
}}

// Add Settings modal before closing </div> (after SideMenu)
<Settings
  isOpen={showSettings}
...
// Update SideMenu onOpenSettings (around line 308)
onOpenSettings={() => {
  setShowSideMenu(false);
  setShowSettings(true);  // ✅ Open local Settings modal
}}
...
**Step 3: ExerciseLog.tsx**
```typescript
// Add import
import Settings from '@/components/Settings';

// Add state to UiState interface or as separate useState
const [showSettings, setShowSettings] = useState(false);
...
// Add Settings modal in the JSX (after SideMenu)
<Settings
  isOpen={showSettings}
  onClose={() => setShowSettings(false)}
/>
...
Testing:
1. Navigate to /programs - click menu → Settings (should open modal)
2. Open a specific program - click menu → Settings (should open modal)
3. On home route - click menu → Settings (should still work)
4. Settings modal should close properly with X button or backdrop click
5. Verify export functionality works from all locations
```
